      ...
      <div className="glass mt-5 space-y-3 rounded-[var(--radius-2xl)] p-4 text-left">
        <div>
          ...
          <textarea
            ...
            className="w-full resize-none rounded-2xl bg-white/5 p-3.5 text-base text-[var(--color-mist)] outline-none placeholder:text-[var(--color-haze)] focus:bg-white/10"
          />
        </div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl bg-white/5 px-3.5 py-2.5 text-sm text-[var(--color-mist)] outline-none placeholder:text-[var(--color-haze)] focus:bg-white/10"
            />
          </div>
          <div>
            <label className="mb-1.5 block text-xs text-[var(--color-haze)]">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
        ...
        {DEMO_CASES.map((c) => (
          <button
            key={c.label}
            onClick={() => fillDemo(c)}
            className="rounded-full border border-white/10 px-3 py-1.5 text-xs text-[var(--color-mist-soft)] transition-colors hover:border-[var(--color-aura)]/60 hover:text-[var(--color-mist)]"
          >
            {c.label}
          </button>
        ))}
